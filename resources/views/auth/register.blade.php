...
@section('title', __('cms.auth.doctor_portal') . ' — ' . __('cms.auth.tab_signup') . ' — CMS')
...
<style>
    ...
    .radio-pill input[type="checkbox"]:checked + label {
        border-color: #8b5cf6;
        color: #fff;
        background: rgba(139, 92, 246, 0.05);
    }
    .radio-pill input[type="checkbox"]:checked + label::before {
        background-color: #8b5cf6;
        box-shadow: 0 0 6px #8b5cf6;
    }
    .radio-pill input[type="checkbox"]:focus-visible + label {
    ...
    <div class="auth-form-container">
        <div class="auth-card large" id="doctorAuthCard">
            ...
            <div class="auth-card-header">
                <h2 class="auth-card-title register-title doctor" id="cardTitle"><i class="bi bi-circle-fill" style="font-size:8px;vertical-align:middle;"></i> {{ strtoupper(__('cms.auth.doc_reg_title')) }}</h2>
                <p class="auth-card-subtitle" id="cardSubtitle">{{ __('cms.auth.fill_details') }}</p>
                @if(session('success'))
            ...
            <div class="auth-tabs">
                <a href="{{ route('login') }}" class="auth-tab" data-panel="panelLogin">{{ __('cms.auth.tab_login') }}</a>
                <a href="{{ route('register') }}" class="auth-tab active doctor-theme" data-panel="panelRegister">{{ __('cms.auth.tab_signup') }}</a>
            </div>

            <div class="form-panels-wrapper" id="formPanelsWrapper">

                <!-- PANEL: LOGIN (Hidden) -->
                <div class="form-panel panel-hidden" id="panelLogin"
                ...
                <!-- PANEL: REGISTER (Visible) -->
                <div class="form-panel panel-visible" id="panelRegister"
                    ...
                    <form class="needs-validation" method="POST" action="{{ route('register') }}" novalidate id="registerForm">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3 position-relative">
                                ...
                                <div class="input-icon-wrapper">
                                    ...
                                    <input type="text" class="form-control @error('phone') is-invalid @enderror" name="phone" id="regPhone"
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3 position-relative">
                                ...
                                <div id="pwStrengthBar" style="height:3px;border-radius:2px;margin-top:.4rem;background:#333;">
                                ...
                                <div class="radio-pill-group">
                                    ...
                                    <div class="radio-pill">
                                        <input
                                            ...
                                            value="{{ $level->id }}"
                                            id="reg_level_{{ $level->id }}"
                                            {{ in_array($level->id, old('requested_levels', [])) ? 'checked' : '' }}
                                        >
                                        <label for="reg_level_{{ $level->id }}">
                                            {{ $level->name }}
                                        </label>
                                    </div>
                                    @endforeach
...
(function () {
    ...
    if (nidInput) {
        nidInput.addEventListener('input', function () { this.value = this.value.replace(/\D/g, '').slice(0, 14); });
    ...
    if (pwInput && bar) {
        pwInput.addEventListener('input', function () {
            const lvl = levels[this.value.length ? scorePassword(this.value) : 0] || levels[0];
    ...
    function checkMatch() {
        ...
        matchLbl.textContent = ok ? '✓ Passwords match' : '✗ Do not match';
        matchLbl.style.color = ok ? '#22c55e' : '#ef4444';
        pwConfirm.classList.toggle('is-invalid', !ok);
        pwConfirm.classList.toggle('is-valid', ok);
    }
    if (pwConfirm) { pwConfirm.addEventListener('input', checkMatch); if(pwInput) pwInput.addEventListener('input', checkMatch); }

    const regForm = document.getElementById('registerForm');
    if (regForm && nidInput) {
        regForm.addEventListener('submit', function(e) {
            if (!NID_RE.test(nidInput.value)) { e.preventDefault(); nidInput.classList.add('is-invalid'); nidInput.focus(); }
        });
    }
})();
</script>
